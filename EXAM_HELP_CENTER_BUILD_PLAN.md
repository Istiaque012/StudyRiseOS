...
| 5 | logging-questions + mock-exams | ✅ done | 2026-07-02 | Both built w/ 7 HTML/CSS mocks (question form + accuracy badge, paste-parser, flag-for-review / mock form, readiness bands, per-subject breakdown + mismatch warning, trend-vs-65% + pace). Facts per tutorial §9 (bands, 72s pace). Full schema, §4 gate + build green. Already in hub hasPart. Committed. **S3–S5 complete: 6 articles + hub re-group done. Next: S6 (plan-and-schedule + staying-on-track).** |
| 6 | plan-and-schedule + staying-on-track | ✅ done | 2026-07-02 | Both built w/ 4 mocks (List view + toggle/filters, task drawer / behind-state card + deficit tile, overload coach card). Sprint view deferred to revision-sprint (linked). Full schema, §4 gate + build green. "Catch-up mode" chip + "10–15 extra questions" line are coaching copy, not claimed app behaviour (noted in PR notes). |
| 7 | dashboard + readiness-and-projections | ✅ done | 2026-07-02 | Both built w/ 6 mocks (KPI tiles+strip, T-minus banner, Go/No-Go / three-numbers card, compliance ring, projection chart). Readiness trio gets one line each on dashboard, full distinction on r-and-p — no duplication. Tutorial states no exact mock minimum for the projection — copy says "several logged mocks", no invented threshold. |
| 8 | history + analytics-and-reports | ✅ done | 2026-07-02 | Both built w/ 3 mocks (calendar+day detail / Focus Areas, report layout). Analytics Pro stated in the lead; charts framed as questions; Progress Report both entry points + light-mode-PDF note; weekly habit framing. §4 gate + build green. |
| 9 | revision-sprint + settings | ✅ done | 2026-07-02 | Both built w/ 5 mocks (Sprint nudge + day strip / 11-tab strip, Exam Setup + Daily Routine panels). Sprint: opt-in only, never auto-activates, ≤28-day Dashboard nudge. Settings is a map — every tab cross-links its guide; Billing uses the compliant trial sentence; no HowTo on settings (reference article). Tutorial silent on deactivating a Sprint — no claim made. |
| 10 | faq + sitemap (register all pages) | ✅ done | 2026-07-02 | faq.html built (7 Q&As, parity, answers link full articles). Hub finalized: all six §3 headings, hasPart = 15 articles. Help-home most-read expanded. sitemap.xml: all 18 help URLs w/ help-og.webp image entries, XML validated. Build green. |
| 11 | brain + wiki updates, finalize hub/home, open PR(s) | ✅ done | 2026-07-02 | Brain: 12 (+18 registry rows, help/help-hub types, link debt), 05 (Help Center section 🟨), new 13_HELP_CENTER.md, CHANGELOG. Wiki: content/Help Center.md + index + log. PR notes S6–S11 appended. Website branch pushed; PR "Content: Exam Help Center — Wave 1" opened. S1's 11 [SCREENSHOT PENDING] markers converted to HTML/CSS mocks (website commit 0ae7254) — no placeholders remain. **Merge gate note: `public/help/images/help-og.webp` does not exist yet** — Istiaque generates it from §6.3; every page references it. |
| 12 | post-merge: live-route check + indexing | 🟨 mostly done | 2026-07-02 | PR #61 squash-merged + deployed. ✅ All 18 URLs verified live (200, self-canonical, one h1, GA — no SPA fallthrough). ✅ Inbound links from /features + AMC article built/pushed → **PR #62 (awaits merge)**. ✅ Sitemap already registered in GSC; write-submit blocked (read-only OAuth scope) but not needed — re-crawl is automatic. ✅ 05 rows flipped to live. ⬜ **help-og.webp still absent** (og:image 404s to SPA shell — Istiaque to generate, §6.3). ⬜ GSC "Request Indexing" is UI-only (no API); index status `unknown to Google` on go-live day, organic crawl pending. |
